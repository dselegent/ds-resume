import React from 'react'
import { Image, Button } from 'antd'

export type CustomTemplateHandle = {
  toCustomTemplate: (() => void) | undefined
}

type CustomTemplatePropsType = {}

const CustomTemplate = forwardRef<CustomTemplateHandle, CustomTemplatePropsType>((props, ref) => {
  const CustomTemplateRef = useRef<HTMLDivElement>(null)
  useImperativeHandle(ref, () => ({
    // 滚动到该元素
    toCustomTemplate: () =>
      CustomTemplateRef.current?.scrollIntoView({
        behavior: 'smooth',
      }),
  }))

  return (
    <section className='min-h-[80vh] w-full bg-white' ref={CustomTemplateRef}>
      <IntroduceTitle title='自定义模板 + 随心所欲' subtitle='自定义模板，随意搭配，自定义主题' />
      <aside className='box-border min-h-[60vh] f-b-c pb-10 pl-[5%] pr-[2%]'>
        <div className='f-c-c overflow-hidden rounded-2.5 shadow-[0_0_70px_rgba(175,50,50,.2)]'>
          <Image
            width='50vw'
            src='https://91huajian.cn/static/images/index-drag-553cb2ca.png'
            loading='lazy'
            alt='customTemplate'
            fallback='data:image/png;base64,'
          />
        </div>
        <div className='flex flex-1 flex-col translate-x-25 justify-center'>
          <h1 className='translate-y-2.5 break-normal bg-[url(https://91huajian.cn/static/images/font-bgc-833dc3d4.png)] bg-clip-text text-justify text-[2vw] lh-normal text-transparent letter-1'>
            丰富的物料模块组件，可随意进行搭配，还可进行主题配置等等
          </h1>
          {/* <p className='translate-y-2.5 text-[.8vw] opacity-0.9 text-gray-400'>
            自定义的模板可保存到
          </p> */}
          <Button type='primary' className='h-13 w-40 translate-y-10 rounded-13 text-base'>
            自定义模板
          </Button>
        </div>
      </aside>
    </section>
  )
})

export default CustomTemplate
